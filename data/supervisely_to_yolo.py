...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Convert Supervisely dataset(s) to YOLO format.")
    p.add_argument("--input_root", required=True, help="Root of Supervisely export (contains meta.json and split dirs)")
    p.add_argument("--output_dir", required=True, help="Destination root for YOLO dataset")
    p.add_argument("--train-splits", type=str, default="ds1_simplex-train,ds2_complex-train", help="Comma-separated list of training split directories to merge")
    p.add_argument("--val-splits", type=str, default="ds1_simplex-test,ds2_complex-test", help="Comma-separated list of validation split directories to merge")
    p.add_argument("--task", choices=["det", "seg"], default="det", help="det for boxes, seg for polygons")
    p.add_argument("--sanity-samples", type=int, default=9, help="Number of random overlay images for visual QC")
    p.add_argument("--sanity-prefix", type=str, default="positive_", help="Prefix for sanity-check sample filenames")
    p.add_argument("--heatmap-grid", type=int, default=200, help="Resolution of the density heat-map")
...
def aggregate_images_and_labels(splits: List[str], input_root: Path) -> Dict[str, List[Tuple[str, str]]]:
    positives = []
    negatives = []
...
def copy_and_create_labels(items: List[Tuple[str, str]], input_root: Path, dest_img_dir: Path, dest_lbl_dir: Path, class_map: Dict[int, int], task: str) -> None:
    ...
    for split, fname in tqdm(items, unit="img", desc="Copying and labeling"):
        ...
        width, height = sup_ann.get("size", {}).get("width"), sup_ann.get("size", {}).get("height")
        ...
        for obj in sup_ann.get("objects", []):
            ...
            norm_pts = [f"{p}/{dim:.6f}" for x, y in pts for p, dim in [(x, width), (y, height)]]
...
def build_heatmap(label_dir: Path, grid: int) -> np.ndarray:
    ...
    for lbl_file in tqdm(list(label_dir.glob("*.txt")), unit="lbl", desc="Heatmap accumulation"):
...
def sanity_check(train_img_dir: Path, train_lbl_dir: Path, out_dir: Path, n_samples: int, prefix: str, heatmap_grid: int) -> None:
    if plt is None:
...
def main() -> None:
    ...
    pos_train, pos_val = train_test_split(agg["positive"], test_size=0.2, random_state=args.seed)
    neg_train, neg_val = train_test_split(agg["negative"], test_size=0.2, random_state=args.seed)
    ...
    if args.sanity_samples > 0:
        ...
        sanity_check(train_img_out, train_lbl_out, sanity_dir, args.sanity_samples, args.sanity_prefix, args.heatmap_grid)
