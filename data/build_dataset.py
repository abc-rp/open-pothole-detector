...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build YOLO dataset from Supervisely export."
    )
    parser.add_argument(
        "--archive-path",
        type=str,
        default="data/potholedatasetninja.tar",
        help="Path to the Supervisely tar archive.",
    )
    parser.add_argument(
        "--dataset-name",
        type=str,
        default="potholedatasetninja",
        help="Name of the dataset directory after extraction.",
    )
    parser.add_argument(
        "--yolo-dir",
        type=str,
        default="yolo_potholes",
    ...
    parser.add_argument(
        ...
        help="Comma-separated list of training splits.",
    )
    parser.add_argument(
        "--val-splits",
        type=str,
        default="ds1_simplex-test,ds2_complex-test",
        help="Comma-separated list of validation splits.",
    )
...
def main():
    ...
    cmd = ["python", "data/data_check_images.py", "--dataset-path", output_dir]
    subprocess.run(cmd, check=True)
