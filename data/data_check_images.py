...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot sample images with bounding boxes to check data."
    )
    parser.add_argument(
        "--dataset-path",
        type=str,
    ...
    parser.add_argument(
        "--num-samples", type=int, default=5, help="Number of random images to check."
    )
    parser.add_argument(
        "--image-prefix",
        type=str,
        default="positive_",
        help="Only check images with this prefix.",
    )
    ...
    for img_path in sample_imgs:
        ...
        for sz in sizes:
            ...
            plot_image_with_boxes(
                str(img_path), str(label_path), class_names, str(out_path), resize=sz
            )
